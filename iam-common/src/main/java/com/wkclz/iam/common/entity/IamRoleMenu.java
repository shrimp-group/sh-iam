 ...
 * @author shrimp
 * @table iam_role_menu (角色-菜单关系) 重新生成代码会覆盖
 */

@Data
@EqualsAndHashCode(callSuper = false)
public class IamRoleMenu extends BaseEntity {
    ...
    public static IamRoleMenu copy(IamRoleMenu source, IamRoleMenu target) {
        if (target == null ) { target = new IamRoleMenu();}
    ...
    public static IamRoleMenu copyIfNotNull(IamRoleMenu source, IamRoleMenu target) {
        if (target == null ) { target = new IamRoleMenu();}
        if (source == null) { return target; }
