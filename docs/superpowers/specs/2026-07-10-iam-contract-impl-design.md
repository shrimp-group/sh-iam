...
**包含**：
- `JwtAuthContract`（iam-sdk）：JWT + Redis 认证实现
...
- `LoggingFilter` 改造：使用契约层抽象
- `SessionHelper` 调用方迁移到 `PrincipalContext`
- `IamSdkConfig` 精简
- 删除旧抽象（`IamAuthFilter`/`IamSsoService`/`SessionHelper`/`UserJwt`/`UserSession`/`SsoFacade`/`SsoFacadeImpl`）
- pom.xml 依赖调整
...
public AuthResult authenticate(HttpServletRequest request) {
    ...
    if (token == null) return null;  // 无 token → 返回 null，过滤器放行 public
...
public Session checkToken(String token, String authIdentifier) {
    ...
    // 3. Redis 会话查询（Redis 异常抛 RuntimeException，非 AuthException）
    String sessionKey = JwtUtil.getTokenRedisKey(token, username);
    String sessionJson;
    try {
    ...
    // 6. authIdentifier 一致性校验（防跨用户冒用）
    //    密码登录：authIdentifier == username
    //    三方登录：authIdentifier == Session.authIdentifier
    //    authIdentifier 非空时，至少匹配其中之一；为空时跳过（向后兼容）
...
| # | 失败场景 | 异常类型 |
|---|---|---|
| 1 | token 为空 | `AuthException(TOKEN_MISSING)` |
| 2 | token 格式错误 | `AuthException(TOKEN_INVALID)` |
| 3 | token 签名无效 | `AuthException(TOKEN_INVALID)` |
| 4 | token 已过期 | `AuthException(TOKEN_EXPIRED)` |
| 5 | Redis 不可用 | `RuntimeException`（系统级错误） |
| 6 | Session 不存在 | `AuthException(SESSION_EXPIRED)` |
| 7 | Session JSON 反序列化失败 | `AuthException(TOKEN_INVALID)` |
| 8 | authIdentifier 不匹配 | `AuthException(TOKEN_INVALID)` |

**关键决策**：
- Redis Session 存储格式不变：继续用 `iam:session:{username}:{tokenMd5}` key + JSON 值。JSON 反序列化为契约层 `Session`。
- `JwtUtil` 保留作为 iam-sdk 内部工具，`UserJwt` 作为内部模型（不对外暴露）。
- Redis 异常抛 RuntimeException 而非 AuthException：契约约定"系统级错误抛 RuntimeException，业务认证失败抛 AuthException"。
- authIdentifier 校验顺序放在最后：需要先从 Redis 取出 Session 才能校验三方场景。
...
2. **`SessionHelper` → `PrincipalContext`**（finally 块）：
...
3. **`fetchRequestLog` 中 SessionHelper 调用迁移**：
...
4. **`IamSdkConfig` 依赖保留**：LoggingFilter 仍需 `IamSdkConfig`（`staticEnabled` / `staticSubfix` 用于静态资源过滤）。
...
**解决方案**：`PrincipalContext` 已扩展 `getPrincipal(request)` / `getSession(request)` 方法，从 request attribute 直接读取（不经 ThreadLocal）。即使 `DefaultAuthFilter` 的 `PrincipalContext.clear()` 已执行 ThreadLocal 清理，request attribute 仍可读。
