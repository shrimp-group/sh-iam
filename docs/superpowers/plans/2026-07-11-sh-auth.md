...
**架构：** 26 个 SPI 契约接口 + 5 个过滤器 + 20 个数据模型（含 AuthMetadata 应用级单例 + SubjectAuthorization 用户级轻量缓存快照）+ SecurityContext + ShAuthAutoConfiguration。过滤器链：RequestWrapper → RequestLog（最外层，含 finally clear）→ SecurityHeader → Authentication → Authorization。LoginService 模板方法编排登录流程。
...
│   │       └── AuthMetadataService.java
│   ├── cache/
│   │   ├── AuthCacheManager.java
│   │   ├── AuthCacheRefreshEvent.java
│   │   └── AuthCacheRefreshListener.java
│   └── filter/                   (6 个过滤器)
...
public class ResolvedAuthorization implements Serializable {
    ...
    private Map<String, List<String>> dataScopes;
...
- 创建：`sh-auth/src/main/java/com/wkclz/auth/model/ResolvedAuthorization.java`
...
```bash
git add sh-auth/src/main/java/com/wkclz/auth/model/SubjectRole.java sh-auth/src/main/java/com/wkclz/auth/model/RoleDataScope.java sh-auth/src/main/java/com/wkclz/auth/model/ApiField.java sh-auth/src/main/java/com/wkclz/auth/model/LoginRecord.java sh-auth/src/main/java/com/wkclz/auth/model/RequestRecord.java sh-auth/src/main/java/com/wkclz/auth/model/SecurityHeaders.java sh-auth/src/main/java/com/wkclz/auth/model/AuthMetadata.java sh-auth/src/main/java/com/wkclz/auth/model/SubjectAuthorization.java sh-auth/src/main/java/com/wkclz/auth/model/ResolvedAuthorization.java
...
- 创建：`sh-auth/src/main/java/com/wkclz/auth/contract/infra/AuthMetadataService.java`
...
import com.wkclz.auth.model.AuthMetadata;

/**
 * 认证元数据服务（加载 RBAC 配置快照）
 */
public interface AuthMetadataService {

    AuthMetadata loadMetadata(String appCode);

    void refreshMetadata(String appCode);
}
```
...
**文件：**
- 创建：`sh-auth/src/main/java/com/wkclz/auth/enums/RefreshScope.java`
- 创建：`sh-auth/src/main/java/com/wkclz/auth/cache/AuthCacheManager.java`
- 创建：`sh-auth/src/main/java/com/wkclz/auth/cache/AuthCacheRefreshEvent.java`
- 创建：`sh-auth/src/main/java/com/wkclz/auth/cache/AuthCacheRefreshListener.java`

- [ ] **步骤 1：编写 RefreshScope 枚举**
...
- [ ] **步骤 2：编写 AuthCacheManager**

```java
package com.wkclz.auth.cache;

import com.wkclz.auth.model.AuthMetadata;
import com.wkclz.auth.model.SubjectAuthorization;
import lombok.extern.slf4j.Slf4j;
...
public class AuthCacheManager {
    ...
    /** 应用级 RBAC 缓存（key = appCode） */
    private final Map<String, AuthMetadata> metadataCache = new ConcurrentHashMap<>();

    /** 用户级授权缓存（key = subjectId） */
    private final Map<String, SubjectAuthorization> subjectAuthCache = new ConcurrentHashMap<>();

    public AuthMetadata getMetadata(String appCode) {
        return metadataCache.get(appCode);
    }

    public void putMetadata(String appCode, AuthMetadata metadata) {
        metadataCache.put(appCode, metadata);
        log.debug("已更新应用缓存: appCode={}", appCode);
    }

    public void evictMetadata(String appCode) {
        metadataCache.remove(appCode);
        log.debug("已清除应用缓存: appCode={}", appCode);
    }

    public SubjectAuthorization getSubjectAuthorization(String subjectId) {
        return subjectAuthCache.get(subjectId);
    }

    public void putSubjectAuthorization(String subjectId, SubjectAuthorization authorization) {
        subjectAuthCache.put(subjectId, authorization);
        log.debug("已更新用户授权缓存: subjectId={}", subjectId);
    }

    public void evictSubjectAuthorization(String subjectId) {
        subjectAuthCache.remove(subjectId);
        log.debug("已清除用户授权缓存: subjectId={}", subjectId);
    }

    public void clearAll() {
        metadataCache.clear();
        subjectAuthCache.clear();
        log.info("已清除全部认证缓存");
    }
}
```

- [ ] **步骤 3：编写 AuthCacheRefreshEvent**
...
public class AuthCacheRefreshEvent extends ApplicationEvent {
    ...
    public AuthCacheRefreshEvent(Object source, RefreshScope scope, String targetCode) {
        super(source);
        this.scope = scope;
...
- [ ] **步骤 4：编写 AuthCacheRefreshListener**
...
public class AuthCacheRefreshListener {
    ...
    @EventListener
    public void onCacheRefresh(AuthCacheRefreshEvent event) {
        log.info("收到缓存刷新事件: scope={}, targetCode={}", event.getScope(), event.getTargetCode());
        switch (event.getScope()) {
            ...
            case APP -> {
                cacheManager.evictMetadata(event.getTargetCode());
                if (authMetadataService != null) {
                    authMetadataService.refreshMetadata(event.getTargetCode());
                }
            }
            case USER -> {
                cacheManager.evictSubjectAuthorization(event.getTargetCode());
            }
        }
    }
}
...
```bash
git add sh-auth/src/main/java/com/wkclz/auth/enums/RefreshScope.java sh-auth/src/main/java/com/wkclz/auth/cache/
git commit -m "feat: add cache infrastructure (RefreshScope enum, AuthCacheManager, AuthCacheRefreshEvent, AuthCacheRefreshListener)"
```
...
public class ShAuthAutoConfiguration {
    ...
    @Bean
    @ConditionalOnMissingBean
    public AuthCacheManager authCacheManager() {
...
1. **枚举 (9/9)**：AccountStatus, AuthErrorType, AuthStatus, CredentialType, FieldPermissionType, MenuType, RefreshScope, MfaType, TokenType
...
3. **模型 (20/20)**：全部已实现（含 AuthMetadata 应用级单例 + SubjectAuthorization 用户级轻量快照 + ResolvedAuthorization 已解析授权）
4. **SPI 接口 (26/26)**：全部已实现，含 AuthMetadataService、DefaultLogoutService 默认实现
5. **缓存基础设施**：RefreshScope 枚举 + AuthCacheManager + AuthCacheRefreshEvent + AuthCacheRefreshListener
6. **LoginService**：模板方法含 7 步流程 + 4 种异常捕获 + recordAttempt
7. **过滤器 (6/6)**：RequestWrapper, RequestLog, SecurityHeader, Authentication, Authorization + FilterOrder 常量
8. **SecurityContext**：含 clear() 方法，RequestLogFilter.finally 中调用
9. **ShAuthAutoConfiguration**：含 FilterRegistrationBean 注册 + DefaultLogoutService Bean + AuthCacheManager Bean
10. **AutoConfiguration.imports**：已配置
11. **零 IAM 依赖**：pom.xml 中不含任何 iam-* 依赖
