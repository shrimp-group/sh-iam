...
**架构：** 25 个 SPI 契约接口 + 5 个过滤器 + 17 个数据模型 + SecurityContext + ShAuthAutoConfiguration。过滤器链：RequestWrapper → RequestLog（最外层，含 finally clear）→ SecurityHeader → Authentication → Authorization。LoginService 模板方法编排登录流程。
...
```bash
git add sh-auth/src/main/java/com/wkclz/auth/model/SubjectRole.java sh-auth/src/main/java/com/wkclz/auth/model/RoleDataScope.java sh-auth/src/main/java/com/wkclz/auth/model/ApiField.java sh-auth/src/main/java/com/wkclz/auth/model/LoginRecord.java sh-auth/src/main/java/com/wkclz/auth/model/RequestRecord.java sh-auth/src/main/java/com/wkclz/auth/model/SecurityHeaders.java
git commit -m "feat: add relation + log + infra models (3 relations + 3 infra models)"
```
