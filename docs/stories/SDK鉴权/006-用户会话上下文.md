...
**作为** 业务开发者，**我希望** 在请求处理过程中能方便地获取当前登录用户的信息（userCode、username、tenantCode 等），**以便**
在业务逻辑中使用用户上下文进行权限判断和数据过滤。
...
- 数据流向：IamAuthFilter 调用 `cacheUserInfo()` → 存入 `request.setAttribute` + `UserContext.setUserInfo()` → 业务代码通过
  `getUserJwt()` / `getUserSession()` 读取
