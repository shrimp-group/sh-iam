...
2. `saveLog(RequestRecord)` 委托给 `IamRequestService.insertLog()`
3. `IamRequestService.insertLog(RequestRecord)` 将 RequestRecord 转为 IamRequestRecord 并入库
4. 字段映射：将 SDK RequestRecord 的 30+ 个字段逐一映射到数据库实体
5. IP 归属地解析：调用 `IpLocalCacheAdapter.offerQueue()` 异步获取
6. 设置审计字段：若 userCode 不为空，设置 createBy/updateBy
7. 调用 `ssoRequestRecordMapper.insertLog()` 入库
...
| SsoRequestRecordMapper | iam-sso/src/main/java/com/wkclz/iam/sso/mapper/SsoRequestRecordMapper.java |
