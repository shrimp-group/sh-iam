...
micro-autotest 是 sh-microapp 的自动化 REST API 测试模块，基于 Spring Boot 4.x，能在运行时扫描项目中的 RESTful
接口，实时生成测试用例并执行，自动 Mock 外部依赖（MyBatis Mapper、Redis、外部 API），最后生成 MD 和 HTML 格式的测试报告。
